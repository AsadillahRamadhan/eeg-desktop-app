...
class RecordCognitiveView(ctk.CTkFrame):
    ...
    def build_ui(self):
        ...
        # Vertical bar charts (4 categories)
    ...
    def tick(self):
        ...
        if label in (0, 1, 2, 3) and pred_ts is not None and pred_ts != self._last_seen_pred_ts:
            ...
            # Only add event if this category hasn't reached 10 yet
    ...
    def _fireworks_frame(self):
        ...
        # Keep spawning extra bursts while celebration window is active.
    ...
    def _spawn_firework_burst(self, label, big=False):
        ...
        # Confetti kecil sebagai aksen ledakan.
    ...
    def draw_chart(self):
        ...
        for i, (label, canvas) in enumerate(zip(self.labels, self.chart_canvases)):
    ...
    def draw_vertical_bar(self, canvas, label, value):
        ...
        for i in range(11):
            ...
            self.chart_canvases[self.labels.index(label)].create_line(
                margin_left, y,
                w - margin_right, y,
                fill="#E0E0E0", width=1
            )
        ...
        if value > 0:
            ...
            # Bar fill
            self.chart_canvases[self.labels.index(label)].create_rectangle(
                margin_left + 5, bar_y,
                w - margin_right - 5, margin_top + chart_h,
                fill="#7B7CFF", outline=""
            )
            ...
            self.chart_canvases[self.labels.index(label)].create_rectangle(
                margin_left + 5, bar_y,
                w - margin_right - 5, margin_top + chart_h,
                fill="", outline="#6B67FF", width=2
            )

            # Value display on top of bar
            self.chart_canvases[self.labels.index(label)].create_text(
                w / 2, bar_y - 8,
                text=str(int(value)),
                fill="#1A1A40",
    ...
    def _draw_fireworks(self, canvas, label, w, h):
        ...
        for p in particles:
            ...
            # Render outside bar area juga, asal masih di canvas.
